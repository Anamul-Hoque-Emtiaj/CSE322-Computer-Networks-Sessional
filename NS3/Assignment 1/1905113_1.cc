...
{
    std::ofstream ofs_thp ("scratch/1905113/static/"+exp_name+"/throughput/"+exp_name+"-"+std::to_string(num)+".dat", std::ios::out | std::ios::app);
    std::ofstream ofs_deliver ("scratch/1905113/static/"+exp_name+"/delivery_ratio/"+exp_name+"-"+std::to_string(num)+".dat", std::ios::out | std::ios::app);
...
void topology(int nNode, int nFlow,int nPPS,int ca, std::string exp_name)
{   
    //std::string phyMode = "DsssRate1Mbps";

    //Config::SetDefault ("ns3::WifiRemoteStationManager::NonUnicastMode", StringValue (phyMode));
    //Config::SetDefault ("ns3::RangePropagationLossModel::MaxRange", DoubleValue (txpDistance));
    std::cout<<"Running for variable "<< exp_name<<": node: "<<2*nNode+2<<", Flow: "<<nFlow<<", Packet per second: "<<nPPS<<", Coverage Area: "<<ca<<std::endl;
   ...
   //  configure the AP (access point) node
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
                                  ...
                                  "DeltaX", DoubleValue(double(txpDistance)/ca),
                                  "DeltaY", DoubleValue(double(txpDistance)/ca),
    ...
    if(exp_name=="node")
    {
        ...
        bottleNeckLink.EnablePcapAll("scratch/1905113/static/"+exp_name+"/pcap/"+exp_name+"-"+std::to_string(nNode*2+2));
    }
    else if(exp_name=="flow")
    {
        Simulator::Schedule (Seconds (1.75), &calMetrix,nFlow,exp_name);
        bottleNeckLink.EnablePcapAll("scratch/1905113/static/"+exp_name+"/pcap/"+exp_name+"-"+std::to_string(nFlow));
    }
    else if(exp_name=="pps")
    {
        Simulator::Schedule (Seconds (1.75), &calMetrix,nPPS,exp_name);
        bottleNeckLink.EnablePcapAll("scratch/1905113/static/"+exp_name+"/pcap/"+exp_name+"-"+std::to_string(nPPS));
    }
    else if(exp_name=="ca")
    {
        Simulator::Schedule (Seconds (1.75), &calMetrix,ca,exp_name);
        bottleNeckLink.EnablePcapAll("scratch/1905113/static/"+exp_name+"/pcap/"+exp_name+"-"+std::to_string(ca));
    }
    else
    {
        Simulator::Schedule (Seconds (1.75), &calMetrix,nNode,exp_name);
        bottleNeckLink.EnablePcapAll("scratch/1905113/static/"+exp_name+"/pcap/pcap");
    }
    ...
    {
        thp_out<<ca<<" "<<throughput<<std::endl;
        dr_out<<ca<<" "<<deliveryRatio<<std::endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    cmd.AddValue("ca", "Coverage Area", ca);
    cmd.Parse(argc, argv);
    ...
    std::string dirSave = "mkdir -p scratch/1905113/static/"+exp_name;
    std::string dirDel="rm -rf scratch/1905113/static/"+exp_name;
    ...
    dirSave = "mkdir -p scratch/1905113/static/"+exp_name+"/throughput";
    if (system (dirSave.c_str ()) == -1)
    {
        exit (1);
    }
    dirSave = "mkdir -p scratch/1905113/static/"+exp_name+"/delivery_ratio";
    if (system (dirSave.c_str ()) == -1)
    {
        exit (1);
    }

    dirSave = "mkdir -p scratch/1905113/static/"+exp_name+"/pcap";
    if (system (dirSave.c_str ()) == -1)
    {
        exit (1);
    }
    ...
    thp_out.open("scratch/1905113/static/"+exp_name+"/throughput.dat", std::ios::out | std::ios::app);
    dr_out.open("scratch/1905113/static/"+exp_name+"/delivery_ratio.dat", std::ios::out | std::ios::app);
    ...
    {      
        for (size_t i=0; i < integersArg.size(); i++)
        {
            ...
            totalTx=0;
            isInit=true;
            topology((nodeCount-2)/2,nodeCount/1.75,pps,ca,exp_name);
        }
    }
    else if(exp_name=="flow")
    {
        ...
        for (size_t i=0; i < integersArg.size(); i++)
        {
            ...
            totalTx=0;
            isInit=true;
            topology(nodeCount,flowCount,pps,ca,exp_name);
        }
    }
    else if(exp_name=="pps")
    {
        for (size_t i=0; i < integersArg.size(); i++)
        {
            int npps = integersArg[i];
            totalRx=0;
            totalTx=0;
            isInit=true;
            topology((node-2)/2,flow,npps,ca,exp_name);
        }
    }
    else if(exp_name=="ca")
    {
        for (size_t i=0; i < integersArg.size(); i++)
        {
            ...
            totalTx=0;
            isInit=true;
            topology((node-2)/2,flow,pps,ca,exp_name);
        }
    }
    else
    {
